document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadSchedules() {
        try {
            ...
            scheduleListBody.innerHTML = '<tr><td colspan="5" class="text-center text-danger">スケジュールの読み込みに失敗しました。</td></tr>';
        }
    }
    ...
    function renderSchedules(schedules) {
        ...
        if (schedules.length === 0) {
            scheduleListBody.innerHTML = '<tr><td colspan="5" class="text-center">登録されているスケジュールはありません。</td></tr>';
            return;
        }
        ...
        schedules.forEach(schedule => {
            ...
            row.innerHTML = `
                ...
                <td class="action-buttons">
                    <button class="btn btn-sm btn-warning edit-btn" data-id="${schedule.id}" data-description="${escapeHtml(schedule.description)}" data-interval="${schedule.interval_minutes ?? ''}" data-active="${schedule.is_active}">編集</button>
    ...
    addForm.addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch(API_BASE, {
                ...
                body: JSON.stringify({ description, interval_minutes }),
            });
            if (!response.ok) {
